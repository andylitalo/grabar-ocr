...
REWRITE_SYSTEM = (
    ...
    "lowercase word (e.g. a proper noun) or lowercase a capital — only swap a letter "
    "for the letter the OCR misread, keeping its case.\n"
...
MINIMAL_EDIT_SYSTEM = (
    ...
    "STRICT RULES:\n"
    "- Only include lines that need a fix. Omit lines that are already correct.\n"
    "- Each key MUST occur verbatim in that line's text, and the edit must be a pure "
    "OCR fix — never a paraphrase, translation, modernization, or case change "
    "(keep each letter's original uppercase/lowercase form).\n"
    "- Do not change notation: Armenian punctuation/abbreviation marks (։ ՟ · ՞ ՛ ՚ "
    "՝), letter-numerals, and trailing hyphens stay as-is."
)
